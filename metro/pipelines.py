...
class MetroPipeline:
    ...
    def open_spider(self, spider):
        self.c.execute("""CREATE TABLE IF NOT EXISTS `metro`
                         (date text, title text, link text, content text)""")

    def process_item(self, item, spider):
        self.c.execute("""SELECT * FROM metro WHERE title = ? AND date = ?""",
        ...
        # Insert values
        self.c.execute("INSERT INTO metro (date, title, link, content)"
